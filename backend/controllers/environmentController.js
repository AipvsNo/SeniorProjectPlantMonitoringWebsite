...
exports.createEnvironment = async (req, res) => {
  try {
    ...
    if (sendCounter % 22 === 0) {
      // 22th reading → persist
      payload = await Environment.create({
        airTemp,
        airPercentHumidity,
        soilTemp,
        soilPercentHumidity,
      ...
      // otherwise just fetch the last saved doc
      payload = {
        airTemp,
        airPercentHumidity,
        soilTemp,
    ...
    // broadcast the payload either way
    ...
    res.status(201).json({
      message:
        sendCounter % 22 === 0
          ? "Reading saved (every 25th)."
          : `Reading skipped (count=${sendCounter}).`,
      data: payload,
    });
  } catch (error) {
