  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        switch (messagePayload.path) {
          case "/environmentData": {
            setSensorData(messagePayload.data);
            console.log("Realtime sensor data:", messagePayload.data);
            break;
          }
          case "/countdown": {
            setSecondsLeft(messagePayload.secondsLeft);
            break;
          }
          default:
    ...
    <div className="w-full p-6 bg-white shadow rounded">
      ...
      {sensorData ? (
        <div className="space-y-2">
          {secondsLeft !== null && (
            <CountdownTimer secondsLeft={secondsLeft} interval={14} />
          )}
